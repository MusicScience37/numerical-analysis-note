...
\section{Bessel 関数}\label{sec:special-function_bessel-function}

\begin{figure}[tp]
    \centering
    \includegraphics[width=0.99\linewidth]{plots/cylindrical-bessel-functions.pdf}
    \caption{$0$ から $5$ 次までの Bessel 関数 $J_n(x)$ （上）と Neumann 関数 $N_n(x)$ （下）}
    \label{fig:special-function_bessel-function}
\end{figure}

\begin{figure}[tp]
    \centering
    \includegraphics[width=0.99\linewidth]{plots/modified-bessel-functions.pdf}
    \caption{$0$ から $5$ 次までの第 1 種変形 Bessel 関数 $I_n(x)$ （上）と第 2 種変形 Bessel 関数 $K_n(x)$ （下）}
    \label{fig:special-function_modified-bessel-function}
\end{figure}
...
の内容をまとめる．
Bessel 関数は円柱座標系における波動方程式の解として現れる．
...
$0$ 次から $5$ 次までの Bessel 関数と Neumann 関数をプロットしたものを
図 \ref{fig:special-function_bessel-function} に示す．

\subsection{変形 Bessel 関数}

Bessel 関数の引数を $iz$ とした以下の関数を第 1 種変形 Bessel 関数と呼ぶ．
\begin{equation}
    I_{\nu}(z) \equiv e^{-i \nu \pi / 2} J_{\nu}(iz)
    = \left(\frac{z}{2}\right)^{\nu}
    \sum_{m = 0}^{\infty} \frac{1}{m! \Gamma(m + \nu + 1)}
    \left(\frac{z}{2}\right)^{2m}
\end{equation}

また，第 1 種変形 Bessel 関数について Neumann 関数と同様の変形をした以下の関数を第 2 種変形 Bessel 関数と呼ぶ．
\begin{equation}
    K_{\nu}(z) \equiv \frac{\pi}{2} \frac{I_{-\nu}(z) - I_{\nu}(z)}{\sin(\nu \pi)}
\end{equation}
Neumann 関数と同様に，整数次の場合は $\nu$ を非整数から近づける極限により定義される．

$0$ 次から $5$ 次までの第 1 種変形 Bessel 関数と第 2 種変形 Bessel 関数をプロットしたものを
図 \ref{fig:special-function_modified-bessel-function} に示す．
