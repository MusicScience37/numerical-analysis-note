...
\index{characteristics method|see{特性曲線法}}

移流項を安定して計算する方法の 1 つとして，移流項のみの時間発展による方程式（移流方程式）
\begin{equation}
    \frac{\partial u}{\partial t} + \bm{v} \cdot \nabla u = 0
\end{equation}
の解を用いる特性曲線法がある \cite{Knabner2003}．
...
このことを利用して，
\begin{equation}
    u(\bm{X}(t), t + \Delta t) = u(\bm{X}(t - \Delta t), t)
\end{equation}
のように曲線 $\bm{X}(t)$ 上をさかのぼった点の値を用いて移流方程式の時間発展を計算することができる．
このような曲線 $\bm{X}(t)$ は特性曲線と呼ばれる．

移流方程式以外の右辺に追加の項を持つ方程式においては，
方程式を移流項とそれ以外に分けて，
それぞれによる時間発展を別々に計算した結果を足し合わせることで
方程式全体の時間発展を計算する operator splitting と呼ばれる手法が用いられる \cite{Press2007}．
