...
\index{すてっぷはば@ステップ幅}
\index{step size|see{ステップ幅}}
ここで，時間の更新幅 $h$ はステップ幅 (step size) と呼ばれる．
Runge-Kutta 法には，
\index{だんすう@段数}
...
半陰的公式と同様に，
上記に含まれる $\bm{f}$ の Jacobian を $(t, \bm{y}(t))$ のときのものに固定することで
計算時間を減らすことが考えられる．
...
\begin{align}
    ...
    \right|_{\bm{k}_i = \bm{k}_i}
      & = I -
    \begin{pmatrix}
        h a_{11} J & h a_{12} J & \cdots & h a_{1s} J \\
        h a_{21} J & h a_{22} J & \cdots & h a_{2s} J \\
        \vdots     & \vdots     & \ddots & \vdots     \\
        h a_{s1} J & h a_{s2} J & \cdots & h a_{ss} J
    \end{pmatrix}
    \label{eq:ode_runge-kutta_implicit-jacobian-fixed}
    \\
    J & = \left. \frac{\partial \bm{f}}{\partial \bm{y}}
    \right|_{t = t, \bm{y} = \bm{y}(t)}
\end{align}
...
この場合 Newton 法を適用するときの Jacobian は式 \eqref{eq:ode_runge-kutta_implicit-jacobian-fixed} と一致する．
...
\footnote{stiffly accurate な公式は，ただ計算が楽になるというだけでなく，%
    追加の条件を満たしていれば L 安定と呼ばれる安定性を満たし，%
    硬い方程式においても安定することが知られている\cite[Section IV.3.]{Hairer1991}．}
．

\subsection{質量行列がある場合}

質量行列 $M$ を用いて微分方程式が
\begin{equation}
    M \dot{\bm{y}} = \bm{f}(t, \bm{y})
\end{equation}
のように表される場合，方程式を以下のように書ける．
\begin{itemize}
    \item 勾配 $\bm{k}_i$ について解く方程式
          \begin{equation}
              M \bm{k}_i = \bm{f}\left(t + b_i h, \bm{y}(t) + h \sum_{j = 1}^i a_{ij} \bm{k}_j \right)
          \end{equation}
    \item 移動量 $\bm{z}_i$ について解く方程式
          \begin{equation}
              M \bm{z}_i = h \sum_{j = 1}^s a_{ij} \bm{f}\left(t + b_j h, \bm{y}(t) + \bm{z}_j \right)
          \end{equation}
\end{itemize}

ここで，前者の方程式は質量行列 $M$ が時間 $t$ や変数 $\bm{y}$ に依存していても使用できるが，
後者の方程式の場合は質量行列 $M$ が定数である必要がある．

\subsection{陰的公式における Newton 法の高速化}

定数の質量行列がある場合の陰的公式における方程式
\begin{equation}
    M \bm{z}_i = h \sum_{j = 1}^s a_{ij} \bm{f}\left(t + b_j h, \bm{y}(t) + \bm{z}_j \right)
\end{equation}
について，文献 \cite{Hairer1991} における高速化の手法を適用する．

まず，Kronecker 積を用いて方程式を以下のように書き換える．
\begin{equation}
    (I \otimes M) \bm{z} = h (A \otimes I) \bm{f}(\bm{z})
\end{equation}

これを Newton 法で解くにあたって，更新量 $\Delta \bm{z}$ を求める方程式は以下になる．
\begin{equation}
    (I \otimes M - h A \otimes J) \Delta \bm{z} = -(I \otimes M) \bm{z} + h (A \otimes I) \bm{f}(\bm{z})
\end{equation}

ここで，行列 $A$ を固有値分解して
$A = T \Lambda T^{-1}$
とする．
ただし，$\Lambda$ は実数の範囲で分解できるように複素数の固有値を
\begin{equation}
    \begin{pmatrix}
        \alpha & \beta  \\
        -\beta & \alpha
...
\end{equation}
のような形式で表したものとする．
これを用いて，以下のように方程式を書き換える．
\begin{equation}
    (I \otimes M - h T \Lambda T^{-1} \otimes J) \Delta \bm{z}
    = -(I \otimes M) \bm{z} + h (T \Lambda T^{-1} \otimes I) \bm{f}(\bm{z})
\end{equation}

変数 $\bm{w} = (T^{-1} \otimes I) \bm{z}$ を導入し，
さらに左から $(h^{-1} \Lambda^{-1} T^{-1} \otimes I)$ を掛けると，以下のようになる．
\begin{equation}
    (h^{-1} \Lambda^{-1} \otimes M - I \otimes J) \Delta \bm{w}
    = -h^{-1} (\Lambda^{-1} \otimes M) \bm{w} + (T^{-1} \otimes I) \bm{f}((T \otimes I) \bm{w})
\end{equation}

左辺の係数行列は以下のような 2 種類のブロックによるブロック対角行列となる．
（$\gamma$ は実数の固有値で，$\hat{\alpha}$ と $\hat{\beta}$ は複素数の固有値の逆数における実部と虚部を示す．）
\begin{gather}
    h^{-1} \gamma^{-1} M -J
    \\
    \begin{pmatrix}
...
は複素数を用いて
\begin{equation}
    (h^{-1} (\hat{\alpha} - i \hat{\beta}) M - J) (\bm{w}_a + i \bm{w}_b)
    = \bm{r}_a + i \bm{r}_b
\end{equation}
と変形することもできる．
計算時のメモリ容量は変化しないが，
LU 分解の計算量が次元の 3 乗オーダーであるため，
次元が少なくなることにより計算量が削減される．

見た目は複雑な更新式だが，ブロックごとに方程式を解くことができるため，
次元の少ない方程式に分解することで全体の計算量を抑えることができる．

定数でない質量行列がある場合は方程式
\begin{equation}
    M \bm{k}_i = \bm{f}\left(t + b_i h, \bm{y}(t) + h \sum_{j = 1}^i a_{ij} \bm{k}_j \right)
\end{equation}
を使用する必要があるため，この方程式に同様の手法を適用することを考える．
まず，簡単のため質量行列を定数として Kronecker 積を用いて方程式を以下のように書き換える．
\begin{equation}
    (I \otimes M) \bm{k} = \bm{f}\left(h (A \otimes I) \bm{k}\right)
\end{equation}
この方程式を Newton 法で解くための更新量 $\Delta \bm{k}$ は
\begin{equation}
    (I \otimes M - h A \otimes J) \Delta \bm{k} = -(I \otimes M) \bm{k} + \bm{f}\left(h (A \otimes I) \bm{k}\right)
\end{equation}
となる．行列 $A$ を固有値分解して
$A = T \Lambda T^{-1}$
とすると，
\begin{equation}
    (I \otimes M - h (T \Lambda T^{-1}) \otimes J) \Delta \bm{k}
    = -(I \otimes M) \bm{k} + \bm{f}\left(h (A \otimes I) \bm{k}\right)
\end{equation}
となる．
変数 $\bm{w} = (T^{-1} \otimes I) \bm{k}$ を導入し，
さらに左から $(h^{-1} \Lambda^{-1} T^{-1} \otimes I)$ を掛けると，
\begin{equation}
    (h^{-1} \Lambda^{-1} \otimes M - I \otimes J) \Delta \bm{w}
    = (h^{-1} \Lambda^{-1} T^{-1} \otimes I)
    \left(-(I \otimes M) \bm{k} + \bm{f}\left(h (A \otimes I) \bm{k}\right) \right)
\end{equation}
となる．
定数でない質量行列を使用する場合，右辺の残差の計算における $M$ の計算は
正しい質量行列を使用するようにすることで，Newton 法を近似的に適用することができる．
...
\end{equation}
のような中央差分による近似を用いて実際の Jacobian の計算を回避する
\cite{Blom2016, Blom2013}．
なお，$\epsilon$ はマシンイプシロン $\epsilon_{machine}$ に対して
\begin{equation}
    \epsilon = \frac{\sqrt[3]{\epsilon_{machine}}}{\|\bm{q}_k\|_2}
\end{equation}
のように選択する．
文献 \cite{Blom2016, Blom2013} においては分子が 3 乗根でなく平方根になっていたが，
一般に中央差分による数値微分では平方根でなく 3 乗根を使用する方が良いことが知られている \cite{Press2007}．
...
ここで，
RBF-FD 法（\ref{chap:pde_rbf-fd} 章）を用いて Laplacian の離散化を行った行列 $J$ を用いて
Newton 法における係数行列 $A = I - h a J$ を $h a = 0.01$ として作成し，GMRES 法を
$\|A\bm{x} - \bm{b}\|_2 / \|\bm{b}\|_2 < 10^{-8}$
となるまで適用し，計算時間を計測した．
その結果が図 \ref{fig:ode_runge-kutta_gmres_subspace-dimension-to-processing-time} である
\footnote{計算に使用したマシンの CPU は Intel(R) Core(TM) Ultra 5 125H であり，%
    コンパイラは Clang 21.1.8 を用いた．%
    ソースコードは，リポジトリ \cite{NumericalCollectionCpp} の%
    コミット \texttt{1ab776c2c47780ad0e2433dc4d70fe4d336abee1} 時点における%
    ファイル \texttt{test/bench/ode/sparse\_linear\_equation\_test.cpp} を使用している．}．
この例では，部分空間の次元数 $m$ を 10 にした場合に最も良い性能が得られている．
