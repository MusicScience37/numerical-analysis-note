...
def load_data() -> pandas.DataFrame:
    ...
    """
    with gzip.open(
        os.path.join(THIS_DIR, "implicit_kaps_problem0.data"), mode="rb"
    ) as file:
    ...
    with gzip.open(
        os.path.join(THIS_DIR, "implicit_kaps_problem3.data"), mode="rb"
    ) as file:
        results = msgpack.unpack(file)
    ...
    with gzip.open(
        os.path.join(THIS_DIR, "implicit_kaps_problem6.data"), mode="rb"
    ) as file:
        results = msgpack.unpack(file)
    data_frame = pandas.concat(
        [
    ...
    with gzip.open(
        # cspell: ignore probleminf
        os.path.join(THIS_DIR, "implicit_kaps_probleminf.data"),
        mode="rb",
    ) as file:
        results = msgpack.unpack(file)
    data_frame = pandas.concat(
        [
            ...
            pandas.DataFrame(
                {
                    "Epsilon": ["0"] * len(results["solver_list"]),
                    "Solver": results["solver_list"],
                    "Err. Tol.": results["tolerance_list"],
                    "Error Rate": results["error_rate_list"],
                    "Time [sec]": results["time_list"],
...
def ode_implicit_kaps_problem_work_error() -> plotly.graph_objects.Figure:
    """ベンチマーク結果をプロットする。
